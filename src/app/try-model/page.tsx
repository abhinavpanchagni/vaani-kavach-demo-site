...
export default function TryModelPage() {
  ...
  return (
    <div className="container mx-auto px-4 py-16 max-w-5xl min-h-[calc(100vh-4rem)] space-y-12">
      <div className="text-center space-y-4">
        <h1 className="text-4xl font-semibold tracking-tight text-foreground">Technical Workbench</h1>
        <p className="text-lg text-muted-foreground font-light">
          Test the core voice models and API exchanges directly.
        </p>
      </div>
      ...
      <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-4 flex items-start gap-4 mx-auto max-w-2xl">
        ...
        <div className="space-y-1">
          <h3 className="font-medium text-amber-500 text-sm">Real API integration in progress</h3>
          <p className="text-amber-500/70 text-xs font-light leading-relaxed">
      ...
      <div className="flex border-b border-white/10 mb-8 max-w-3xl mx-auto">
        <button 
        ...
        <AnimatePresence mode="wait">
            ...
            <motion.div key="model" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="grid md:grid-cols-2 gap-12 items-start">
              <div className="space-y-6">
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-muted-foreground font-light">Record or upload an audio clip for real inference.</p>
                </div>
                
                <div className="space-y-4">
                  <div className="flex flex-col items-center justify-center h-48 border border-dashed border-white/10 rounded-xl bg-white/[0.02]">
                    {isRecording ? (
                      ...
                      <div className="flex flex-col items-center gap-3 text-muted-foreground/50">
                        ...
                        <span className="text-sm font-light">Use your microphone to record</span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <Button 
                      ...
                      className={`gap-2 ${!isRecording && 'border-white/10 hover:bg-white/[0.02] bg-transparent'}`}
                    ...
                    <Button variant="outline" className="gap-2 border-white/10 hover:bg-white/[0.02] bg-transparent" onClick={handleUploadClick}>
              ...
              <div className="space-y-6">
                ...
                <div className="h-[288px] border border-white/5 bg-[#0A0A0C] rounded-xl flex flex-col p-6 overflow-hidden relative">
                  {result ? (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="h-full flex flex-col">
                      <div className="flex-1 flex flex-col items-center justify-center text-center space-y-4">
                        <AlertCircle className="w-8 h-8 text-muted-foreground/30" />
                        <p className="text-sm text-muted-foreground font-light">{result.error}</p>
                        <span className="text-[10px] uppercase tracking-widest text-muted-foreground/50 font-medium">API Disconnected</span>
                      </div>
                    </motion.div>
                  ) : (
                    <div className="flex-1 flex flex-col items-center justify-center text-muted-foreground/30">
                      ...
                      <p className="text-sm font-light">Awaiting submission</p>
                    </div>
                  )}
                </div>
              </div>
          ...
          {activeTab === "api" && (
            <motion.div key="api" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-8">
              <div className="flex flex-col items-center justify-center py-16 text-center space-y-4 bg-white/[0.01] border border-dashed border-white/10 rounded-xl">
          ...
          {activeTab === "receipt" && (
            <motion.div key="receipt" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-8">
              <div className="flex flex-col items-center justify-center py-16 text-center space-y-4 bg-white/[0.01] border border-dashed border-white/10 rounded-xl">
                <FileKey className="w-12 h-12 text-muted-foreground/20" />
